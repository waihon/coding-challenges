...
def is_palindrome(s)
  ...
  left = right = (s.length / 2) # Ruby integer division returns an integer
  # When there is an even number of characters, `left` should start at the
  # character to the left of the `righ`.
  left = right - 1 if s.length.even?
